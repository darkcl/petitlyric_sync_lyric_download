    ...
    for root_dir, sub_dir, filenames in os.walk(directory):
        for filename in filenames:
            ...
            try:
                ...
                if lyric_string != "":
                    fout = codecs.open(
                        os.path.join(root_dir, filename_bare +
                                     ".lrc"), "w", "utf-8"
                    )
...
def get_lyric_base64(album, artist, title, lyricsType_request=3):
    ...
    response_body = requests.post(
        lyrics_url, data=request_body, headers=request_header)
    song_xml_tree = ET.fromstring(response_body.text)
    try:
        lyrics_type = song_xml_tree.find("songs").find(
            "song").find("lyricsType").text
        lyrics_base64 = song_xml_tree.find(
            "songs").find("song").find("lyricsData").text
...
def find_lyric(path_to_music_file):
    ...
    if lyrics_type == "2":
        [lyrics_type, lyrics_text_base64] = get_lyric_base64(
            album, artist, title, 1)
...
def lsy_decoder(
    lsy_base64_lyric, lyrics_text_base64
):  # Assumes running on little endian system
    import binascii
    import struct
    import base64
    import numpy as np
    ...
    protection_id = np.uint16(
        int.from_bytes(
            lyrics_encrypted[0x1A: 0x1A + 2], byteorder="little", signed=False
        )
    )
    ...
    line_count = int.from_bytes(
        lyrics_encrypted[0x38: 0x38 + 4], byteorder="little", signed=False
    )
    line_length = int.from_bytes(
        lyrics_encrypted[0x42: 0x42 + 2], byteorder="little", signed=False
    )  # Seems to be 64 for all tested files
    elapsed_time = 0
